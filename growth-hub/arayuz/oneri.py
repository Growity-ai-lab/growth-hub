...
def oneri_uret(brief, karne, sz):
    """brief={sektor_l2, amac, toplam_butce, istenen_format_cesidi?} → plan sözlüğü."""
    ...
    # --- aday havuzu: sektör + amaç uyumu (amaç uyumu sözlükten türetilir) ---
    amac_video = _amac_video_haritasi(sz)
    video_brief = amac_video.get(brief['amac'], False)
    uyarilar = []

    def amac_uyar(c):
        vc = sz.yayin_turu(c.get('ornek_tur') or '')['video']
        return vc == video_brief
    ...
    # --- min_format_cesidi: eksik reklam modeli varsa en iyi fayda'lı adayı çekirdeğe ekle ---
    def modeller(cells):
    ...
    return dict(
        ...
        uyari_esigi=uyari_esigi, satirlar=plan,
